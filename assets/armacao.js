(() => {
    ...
    function zoomPercent() {
        return clamp(Math.round((CAMERA_BASE_DISTANCE / Math.max(0.001, state.camera.distance)) * 100), 10, 1500);
    }

    function syncZoomPreset() {
        const percent = zoomPercent();
        const allowed = new Set(['100', '200', '300', '800', '1500']);
        const exact = String(percent);
        el.riggingZoomPreset.value = allowed.has(exact) ? exact : 'custom';
    }

    function setZoomPercent(percent) {
        const numeric = clamp(Number(percent || 100), 10, 1500);
